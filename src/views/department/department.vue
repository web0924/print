...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      // })
      axios.post('/smartprint/company/office/get-offices', qs.stringify(vm.listQuery))
        .then(res => {
    ...
    getListLen() {
      ...
      params.isSum = 1
      axios.post('/smartprint/company/office/get-offices', qs.stringify(params)).then(res => {
    ...
    editView() {
      ...
      if (id) {
        axios.post('/smartprint/company/office/get-office', qs.stringify({ officeId: id })).then(res => {
          console.log(res)
          if (res.data.code !== 0) return this.$message.error(res.data.msg)
    ...
    handleEdit(index, { id }) {
      ...
      this.$router.push({ path: '/department/department', query: { extra: 'edit', id } }); // 带参跳转
    },
    // 编辑上传
    onEditSubmit() {
      this.roleTemp2.officeId = this.roleTemp2.id
      axios.post('/smartprint/print-room/office/update-office', qs.stringify(this.roleTemp2)).then(res => {
        if (res.data.code !== 0) return this.$message.error(res.data.msg)
    ...
    handleDelete(index, { id }) {
      ...
      axios.post('/smartprint/print-room/office/delete-office', qs.stringify(id)).then(res => {
        if (res.data.code !== 0) return this.$message.error(res.data.msg)
    ...
    onAddSubmit() {
      ...
      axios.post('/smartprint/print-room/office/create-office', qs.stringify(vm.roleTemp)).then(res => {
