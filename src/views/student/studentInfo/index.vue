<template>
  <div class="app-container">
    ...
    <!-- 表格 -->
    <el-table ref="multipleTable" @selection-change="handleSelectionChange" :data="list" v-loading.body="listLoading" element-loading-text="" border fit highlight-current-row  >
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      global.get(api.studentList, { params: vm.listQuery }, res => {
    ...
    delFun(row) {
      const vm = this;
      global.get(api.delStudents, { params: { id: row._id } }, res => {
              // console.log('-------获取到数据：',JSON.stringify(res) )
        vm.getList();
      }, res => {
        alert('删除报错')
    ...
    handleDelAll() {
      ...
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        type: 'warning'
            ...
            // 前端依次执行单个删除操作
        for (const index in vm.multipleSelection) {
