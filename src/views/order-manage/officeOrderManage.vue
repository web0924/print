<template>
  <div class="app-container">
    <div v-show="viewType === 0" style="padding-bottom:30px">
      <div style="min-height:120px;padding-left:20px">
        ...
        <div class="filter-container">
          ...
          <el-select clearable class="filter-item" style="width: 200px" v-model="listQuery.officeId" size="small"
            placeholder="按科室筛选">
          ...
          <el-date-picker value-format="yyyy-MM-dd 23:59:59" class="filter-item" v-model="listQuery.endTime" type="date"
            size="small" placeholder="结束日期">
          </el-date-picker>
      ...
      <el-table :data="list" ref="tableListRef" v-loading.body="listLoading" element-loading-text=""
        @sort-change="tableSortChange" @selection-change="selectionTableChange" :cell-class-name="tableCellClassName"
        border fit highlight-current-row>
        <el-table-column v-if="receiveGroupVisible" :selectable="selectableHandle" type="selection" width="55">
        </el-table-column>
        ...
        <el-table-column label="科室名称" width="">
          <template slot-scope="scope">
        ...
        <el-table-column align="center" width="320" label="操作">
          <template slot-scope="scope">
            <el-button style="margin-left:50px" v-show="scope.row.status == 'DaiJieDan'" type="primary" plain
              @click="tableOrderStatuHandle('YiJieDan', scope.row, scope.$index)" size="small">确认接单</el-button>
            <el-button style="margin-left:50px" v-show="scope.row.status == 'YiJieDan'" type="primary" plain
              @click="tableOrderStatuHandle('DaiQueRen', scope.row, scope.$index)" size="small">请用户确认</el-button>
            <el-button style="margin-left:50px" v-show="scope.row.status == 'YiQueRen'" type="primary" plain
              @click="tableOrderStatuHandle('ShengChanZhong', scope.row, scope.$index)" size="small">确认生产</el-button>
            <el-button style="margin-left:50px" v-show="scope.row.status == 'ShengChanZhong'" type="primary" plain
            ...
            <el-button v-show="scope.row.status == 'YiShangJia'" type="primary" @click="ensureLingquHandle(scope.row)"
        ...
        <el-form :disabled="isEditOrder" class="small-space" :model="roleTemp" label-position="left" label-width="120px"
          ...
          <div v-if="roleTemp.type == 'Office'">
            <el-form-item label="科室：">
              ...
              <el-select disabled clearable class="filter-item form-item-width" placeholder="无"
          ...
          <el-form-item label="附件：">
            ...
            <el-upload :show-file-list="false" action="#" :on-preview="handlePreview" :on-remove="handleRemove"
              :before-remove="beforeRemove" multiple :limit="10" :http-request="uploadHttpRequest"
            ...
            <div class="class-table-box">
              ...
              <div class="class-table-td" v-for="(item, index) in (roleTemp.attachmentFiles)" :key="item.no">
                <a :href="'https://dev.renx.cc/' + item.url" target="_blank" class="text-overflow"
                  style="width:80%;color:#0053B5">{{  item.originalName  }}</a>
          ...
          <div style="margin-top:20px">
            ...
            <el-form-item label="上传小样：">
              <div style="border:1px solid #E2E2E2;padding:20px">
                ...
                <el-upload action="/smartprint/upload-file" :on-preview="handlePreview" :on-remove="handleSimpalRemove"
                  :before-remove="beforeRemove" :on-success="succcessHandle" multiple :limit="10"
    ...
    <el-dialog title="IC卡验证" @close="dialogICVisible = false" :visible="dialogICVisible">
      <p>{{  this.tips  }}</p>
      <el-steps :active="active" finish-status="success">
        <el-step description="请将IC卡放入感应区内" title="连接设备"></el-step>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button :disabled="!isValidSuccess" @click="ensureOrderHnadle('YiLingQu')" type="primary">领取</el-button>
      </div>
    </el-dialog>
    <!-- 通知单 -->
...
export default {
  ...
  computed: {
    ...
    // 订单是否可编辑
    isEditOrder() {
      const status = ['ShengChanZhong', 'YiShangJia', 'YiLingQu', 'YiCheXiao']
  ...
  watch: {
    ...
    //       if (document.getElementsByClassName('el-message--warning').length === 0) this.$message.warning('订单还未上架')
    //       return
    //     }
