    ...
    <div v-show="viewType===0"
      ...
      <div style="height:120px;padding-left:20px">
        <div style="height:60px;display:flex;align-items:center">
          <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item :to="{ path: '/subject/subject' }">学科列表</el-breadcrumb-item>
          </el-breadcrumb>
        </div>
        <!-- 搜索条件 -->
      ...
      <el-table ref="multipleTable"
        ...
        <el-table-column label="学科类型"
                         width="">
          <template slot-scope="scope">
            {{typeOptions.filter(item=>item.key===scope.row.type)[0].name}}
          </template>
        </el-table-column>
    ...
    <div v-show="viewType==='add'">
      <div style="padding-left:20px">
        <div style="height:60px;display:flex;align-items:center">
          <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item :to="{ path: '/subject/subject' }">学科列表</el-breadcrumb-item>
            <el-breadcrumb-item>添加学科</el-breadcrumb-item>
          </el-breadcrumb>
        </div>
      </div>
      <p style="height:15px;width:100%;background:#F5F5F5;margin:0"></p>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      // })
      axios.post('/smartprint/print-room/subject/get-subjects', qs.stringify(vm.listQuery))
        .then(res => {
          if (res.data.code !== 0) return this.$message.error(res.data.msg)
          vm.list = res.data.data.subjects;
    ...
    getListLen() {
      ...
      params.isSum = 1
      axios.post('/smartprint/print-room/subject/get-subjects', qs.stringify(params)).then(res => {
    ...
    editView() {
      ...
      if (id) {
        axios.post('/smartprint/print-room/subject/get-subject', qs.stringify({ subjectId: id })).then(res => {
          console.log(res)
          if (res.data.code !== 0) return this.$message.error(res.data.msg)
          this.roleTemp2 = res.data.data.subject
        }).catch(err => console.log(err))
      }
    },
    // 编辑
    handleEdit(index, { id }) {
      ...
      this.$router.push({ path: '/subject/subject', query: { extra: 'edit', id } }); // 带参跳转
    },
    // 编辑上传
    onEditSubmit() {
      this.roleTemp2.subjectId = this.roleTemp2.id
      axios.post('/smartprint/print-room/subject/update-subject', qs.stringify(this.roleTemp2)).then(res => {
        if (res.data.code !== 0) return this.$message.error(res.data.msg)
    ...
    handleDelete(index, { id }) {
      ...
      axios.post('/smartprint/print-room/subject/delete-subject', qs.stringify({ subjectId: id })).then(res => {
        if (res.data.code !== 0) return this.$message.error(res.data.msg)
    ...
    onAddSubmit() {
      ...
      axios.post('/smartprint/print-room/subject/create-subject', qs.stringify(vm.roleTemp)).then(res => {
